...
Hermes is the operator shell. egc is the reusable system behind it.
...
- egc skills, agents, commands, hooks, and MCP configs from this repo
...
Use Hermes as the front door and egc as the reusable workflow substrate.
...
Telegram / CLI / TUI
      ...
      Hermes
        ↓
 egc skills + hooks + MCPs + generated workflow packs
        ↓
...
- `~/.hermes/skills/egc-imports/`
  - egc skills copied in for Hermes-native use
...
- egc for skills, rules, prompts, and cross-harness conventions
...
0. Run `egc migrate audit --source ~/.hermes` first to inventory the legacy workspace and see which parts already map onto egc.
0.5. Plan and scaffold migration artifacts before importing anything:
   - generate reviewable plans with `egc migrate plan` and `egc migrate scaffold`
   - scaffold reusable legacy skills with `egc migrate import-skills --output-dir migration-artifacts/skills`
   - scaffold tool translation templates with `egc migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `egc migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `egc migrate import-schedules --dry-run`
   - preview gateway dispatch with `egc migrate import-remote --dry-run`
   - preview safe env/service context with `egc migrate import-env --dry-run`
   - import sanitized workspace memory with `egc migrate import-memory`
1. Install egc and verify the baseline harness setup with `node tests/run-all.js`; the expected result is a zero-failure test summary.
2. Install Hermes and point it at egc-imported skills.
...
EGC v1.0.0 documents the Hermes surface and ships launch collateral now.
