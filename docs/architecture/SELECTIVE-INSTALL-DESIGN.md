# egc Selective Install Design

## Purpose

This document defines the user-facing selective-install design for egc.
...
- how users choose egc components
- what the CLI should feel like
- what config file should exist
- how installation should behave across harness targets
- how the design maps onto the current egc codebase without requiring a rewrite

## Problem

Today egc still feels like a large payload installer even though the repo now
...
The selective-install system should make egc feel composable instead of
all-or-nothing.
...
1. Let users install a small default egc footprint quickly.
...
3. Keep one consistent UX across gemini, Cursor, Antigravity, Codex, and
   OpenCode.
4. Keep installs inspectable, repairable, and uninstallable.
5. Preserve backward compatibility with the current `egc-install typescript`
   style during rollout.

## Non-Goals

- packaging egc into multiple npm packages in the first phase
...
A user should be able to get a useful egc install with one command:
...
These are the default egc building blocks:
...
Capability packs are cross-cutting egc feature bundles.
...
- `core`
  minimal baseline, safe default for most users trying egc
- `developer`
...
```bash
egc install [--target <target>] [--profile <name>] [--with <component>]... [--without <component>]... [--config <path>] [--dry-run] [--json]
```
...
```bash
egc plan [same selection flags as install]
```
...
- `target`
  selected harness target such as `gemini`, `cursor`, or `antigravity`
- `profile`
...
The commands differ in action, not in how egc understands the selected install.
...
- `egc-install.json` config file parsing
...
## Mapping To Current egc Manifests
...
That is the shortest path from the current egc codebase to a real selective
install experience that feels like egc 2.0 instead of a large legacy installer.
