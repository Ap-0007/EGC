...
egc is the reusable workflow layer. Harnesses are execution surfaces.
...
|---------|---------------|-----------------|----------------|
| Skills | `skills/*/SKILL.md` | gemini plugin, Codex plugin, `.agents/skills`, Cursor skill copies, OpenCode plugin/config | Supported with harness-specific packaging |
| Rules and instructions | `rules/`, `AGENTS.md`, translated docs | gemini rules install, Codex `AGENTS.md`, Cursor rules, OpenCode instructions | Supported, but not identical across harnesses |
| Hooks | `hooks/hooks.json`, `scripts/hooks/` | gemini native hooks, OpenCode plugin events, Cursor hook adapter | Hook-backed in gemini/OpenCode/Cursor; instruction-backed in Codex |
| MCPs | `.mcp.json`, `mcp-configs/` | Native MCP config import per harness | Supported where the harness exposes MCP |
| Commands | `commands/`, CLI scripts | gemini slash commands, compatibility shims, CLI entrypoints | Supported, but command semantics vary |
| Sessions | `egc2/`, session adapters, orchestration scripts | TUI/daemon, tmux/worktree orchestration, harness-specific runners | Alpha |

## What Travels Unchanged

`SKILL.md` is the most portable unit.

A good egc skill should:
...
Hermes is not the public egc runtime.

Hermes is an operator shell that can consume egc assets:

- import selected egc skills into a Hermes skills directory
- use egc MCP conventions for tool access
- route chat, CLI, cron, and handoff workflows through reusable egc patterns
- distill repeated local operator work back into sanitized egc skills
...
OpenCode gets the skill through the OpenCode package/plugin surface. Event handling can reuse egc hook logic through the adapter layer, while the skill text stays unchanged.
