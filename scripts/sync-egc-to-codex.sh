...
BACKUP_DIR="$CODEX_HOME/backups/egc-$STAMP"
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# EGC Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the EGC `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
require_path "$REPO_ROOT/AGENTS.md" "EGC AGENTS.md"
require_path "$AGENTS_CODEX_SUPP_SRC" "EGC Codex AGENTS supplement"
require_path "$CODEX_AGENTS_SRC" "EGC Codex agent roles"
require_path "$PROMPTS_SRC" "EGC commands directory"
require_path "$BASELINE_MERGE_SCRIPT" "EGC Codex baseline merge script"
require_path "$HOOKS_INSTALLER" "EGC global git hooks installer"
require_path "$SANITY_CHECKER" "EGC global sanity checker"
require_path "$CURSOR_RULES_DIR" "EGC Cursor rules directory"
require_path "$CONFIG_FILE" "Codex config.toml"
require_path "$MCP_MERGE_SCRIPT" "EGC MCP merge script"
...
EGC_BEGIN_MARKER="<!-- BEGIN EGC -->"
EGC_END_MARKER="<!-- END EGC -->"

compose_egc_block() {
  printf '%s\n' "$EGC_BEGIN_MARKER"
  cat "$AGENTS_ROOT_SRC"
  printf '\n\n---\n\n'
  printf '# Codex Supplement (From EGC .codex/AGENTS.md)\n\n'
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  replace_egc_section() {
    # Replace the EGC block between markers in $AGENTS_FILE with fresh content.
    # Uses awk to correctly handle all positions including line 1.
    local tmp
    tmp="$(mktemp)"
    local egc_tmp
    egc_tmp="$(mktemp)"
    compose_egc_block > "$egc_tmp"
    awk -v begin="$EGC_BEGIN_MARKER" -v end="$EGC_END_MARKER" -v egc="$egc_tmp" '
      { gsub(/\r$/, "") }
      $0 == begin { skip = 1; while ((getline line < egc) > 0) print line; close(egc); next }
      $0 == end   { skip = 0; next }
      !skip        { print }
    ' "$AGENTS_FILE" > "$tmp"
    cat "$tmp" > "$AGENTS_FILE"
...
write_extension_prompt "egc-tool-run-tests.md" <<EOF
# EGC Tool Prompt: run-tests
...
write_extension_prompt "egc-tool-check-coverage.md" <<EOF
# EGC Tool Prompt: check-coverage

Analyze coverage and compare it to an 80% threshold (or a threshold I specify).

## Instructions
1. Find existing coverage artifacts first (${EGC_COVERAGE_DIR:-coverage}/coverage-summary.json, ${EGC_COVERAGE_DIR:-coverage}/coverage-final.json, .nyc_output/coverage.json).
...
write_extension_prompt "egc-tool-security-audit.md" <<EOF
# EGC Tool Prompt: security-audit
...
write_extension_prompt "egc-rules-pack-common.md" <<EOF
# EGC Rule Pack: common (optional)

Apply EGC common engineering rules for this session. Use these files as the source of truth:
...
write_extension_prompt "egc-rules-pack-typescript.md" <<EOF
# EGC Rule Pack: typescript (optional)

Apply EGC common rules plus TypeScript-specific rules for this session.

## Common
Use \`$PROMPTS_DEST/egc-rules-pack-common.md\`.
...
write_extension_prompt "egc-rules-pack-python.md" <<EOF
# EGC Rule Pack: python (optional)

Apply EGC common rules plus Python-specific rules for this session.
...
write_extension_prompt "egc-rules-pack-golang.md" <<EOF
# EGC Rule Pack: golang (optional)

Apply EGC common rules plus Go-specific rules for this session.
...
write_extension_prompt "egc-rules-pack-swift.md" <<EOF
# EGC Rule Pack: swift (optional)

Apply EGC common rules plus Swift-specific rules for this session.
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  EGC_GLOBAL_HOOKS_DIR="${EGC_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
