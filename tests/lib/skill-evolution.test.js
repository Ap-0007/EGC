...
function runTests() {
  ...
  const learnedRoot = path.join(homeDir, '.gemini', 'skills', 'learned');
  const importedRoot = path.join(homeDir, '.gemini', 'skills', 'imported');
  const runsFile = path.join(homeDir, '.gemini', 'state', 'skill-runs.jsonl');
  ...
  try {
    ...
    if (test('ignores malformed JSONL rows when reading execution records', () => {
      const malformedRunsFile = path.join(homeDir, '.gemini', 'state', 'malformed-skill-runs.jsonl');
      fs.writeFileSync(
        malformedRunsFile,
        `${JSON.stringify({
          skill_id: 'alpha',
    ...
    if (test('treats an unsnapshotted SKILL.md as v1 and orders last_run by actual time', () => {
      ...
      const offsetRunsFile = path.join(homeDir, '.gemini', 'state', 'offset-skill-runs.jsonl');
