...
const { createClaudeHistoryAdapter } = require('../../scripts/lib/session-adapters/egc-history');
...
test('dmux adapter normalizes orchestration snapshots into canonical form', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-recordings-'));
  ...
  try {
    ...
    const adapter = createDmuxTmuxAdapter({
      ...
      collectSessionSnapshotImpl: () => ({
        ...
        coordinationDir: '/tmp/.gemini/orchestration/workflow-visual-proof',
        repoRoot: '/tmp/repo',
        targetType: 'plan',
        sessionActive: true,
        paneCount: 1,
        ...
        workers: [{
          ...
          workerDir: '/tmp/.gemini/orchestration/workflow-visual-proof/seed-check',
          status: {
            state: 'running',
    ...
    assert.strictEqual(snapshot.schemaVersion, 'egc.session.v1');
    ...
    const adapter = createDmuxTmuxAdapter({
      ...
      collectSessionSnapshotImpl: () => ({
        ...
        workers: [{
          ...
          workerSlug: 'proof',
          workerDir: '/tmp/.gemini/orchestration/workflow-visual-proof/proof',
          status: {
            state: 'completed',
...
test('egc-history adapter loads the latest recorded session', () => {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-adapter-home-'));
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-recordings-'));
  ...
  try {
    withHome(homeDir, () => {
      ...
      const snapshot = adapter.open('egc:latest').getSnapshot();
      const recordingPath = getFallbackSessionRecordingPath(snapshot, { recordingDir });
      const persisted = JSON.parse(fs.readFileSync(recordingPath, 'utf8'));

      assert.strictEqual(snapshot.schemaVersion, 'egc.session.v1');
      assert.strictEqual(snapshot.adapterId, 'egc-history');
      ...
      assert.strictEqual(snapshot.workers[0].worktree, '/tmp/egc-worktree');
      assert.strictEqual(snapshot.workers[0].runtime.kind, 'egc-session');
...
test('adapter registry routes plan files to dmux and explicit egc targets to history', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-registry-repo-'));
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-registry-home-'));
  const sessionsDir = path.join(homeDir, '.gemini', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
  ...
  try {
    withHome(homeDir, () => {
      const registry = createAdapterRegistry({
        adapters: [
          createDmuxTmuxAdapter({
            ...
            collectSessionSnapshotImpl: () => ({
              sessionName: 'workflow-visual-proof',
              coordinationDir: path.join(repoRoot, '.gemini', 'orchestration', 'workflow-visual-proof'),
              repoRoot,
              targetType: 'plan',
              sessionActive: false,
              paneCount: 0,
      ...
      const claudeSnapshot = registry.open('egc:latest', { cwd: repoRoot }).getSnapshot();

      assert.strictEqual(dmuxSnapshot.adapterId, 'dmux-tmux');
      assert.strictEqual(claudeSnapshot.adapterId, 'egc-history');
    });
  } finally {
...
test('adapter registry resolves structured target types into the correct adapter', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-typed-repo-'));
  const planPath = path.join(repoRoot, 'workflow.json');
  fs.writeFileSync(planPath, JSON.stringify({
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-typed-home-'));
  const sessionsDir = path.join(homeDir, '.gemini', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
    path.join(sessionsDir, '2026-03-13-z9y8x7w6-session.tmp'),
  ...
  try {
    withHome(homeDir, () => {
      const registry = createAdapterRegistry({
        adapters: [
          createDmuxTmuxAdapter({
            ...
            collectSessionSnapshotImpl: () => ({
              sessionName: 'workflow-typed-proof',
              coordinationDir: path.join(repoRoot, '.gemini', 'orchestration', 'workflow-typed-proof'),
              repoRoot,
              targetType: 'plan',
              sessionActive: true,
              paneCount: 0,
      ...
      const claudeSnapshot = registry.open({ type: 'egc-history', value: 'latest' }, { cwd: repoRoot }).getSnapshot();

      assert.strictEqual(dmuxSnapshot.adapterId, 'dmux-tmux');
      assert.strictEqual(dmuxSnapshot.session.sourceTarget.type, 'plan');
      assert.strictEqual(claudeSnapshot.adapterId, 'egc-history');
      assert.strictEqual(claudeSnapshot.session.sourceTarget.type, 'egc-history');
...
test('default registry forwards a nested state-store writer to adapters', () => {
  ...
  try {
    withHome(homeDir, () => {
      const snapshot = inspectSessionTarget('egc:latest', {
        cwd: process.cwd(),
        stateStore
      });
      ...
      assert.strictEqual(stateStore.sessions.persisted[0].snapshot.adapterId, 'egc-history');
...
test('adapter registry lists adapter metadata and target types', () => {
  ...
  assert.ok(ids.includes('egc-history'));
  assert.ok(ids.includes('dmux-tmux'));
  assert.ok(
    adapters.some(adapter => adapter.id === 'egc-history' && adapter.targetTypes.includes('egc-history')),
    'egc-history should advertise its canonical target type'
  );
  assert.ok(
...
test('canonical snapshot validation rejects malformed required fields', () => {
  const invalidCases = [
    ...
    [canonicalSnapshot({ schemaVersion: 'egc.session.v0' }), /Unsupported canonical session schema version/],
...
test('egc history normalization falls back to filename ids and empty metadata defaults', () => {
  ...
  assert.deepStrictEqual(pathOnly.workers[0].intent.seedPaths, ['README.md', 'scripts/egc.js']);
...
test('fallback recordings sanitize paths, use env dirs, and preserve changed history', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-recordings-env-'));
  ...
  try {
    ...
    assert.strictEqual(persisted.schemaVersion, 'egc.session.recording.v1');
...
test('persistence supports skip mode, writer variants, and missing state-store fallback', () => {
  ...
  const noWriterDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-no-writer-'));
  const missingModuleDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-session-missing-module-'));
  try {
...
test('persistence only falls back when the state-store module is missing', () => {
  const snapshot = {
    ...
    workers: [{
      ...
      artifacts: {
        sessionFile: process.env.EGC_ARTIFACTS_DIR ? path.join(process.env.EGC_ARTIFACTS_DIR, 'session.tmp') : '/tmp/session.tmp',
        context: null
      }
    }],
    aggregates: {
