...
test('fails fast for an unreadable task file and records failure artifacts', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-orch-worker-'));
  const orchestrationDir = process.env.EGC_ARTIFACTS_DIR || '.orchestration';
  const handoffFile = path.join(tempRoot, orchestrationDir, 'docs', 'handoff.md');
  const statusFile = path.join(tempRoot, orchestrationDir, 'docs', 'status.md');
  const missingTaskFile = path.join(tempRoot, orchestrationDir, 'docs', 'task.md');
