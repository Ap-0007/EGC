...
function createTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'egc-loop-status-home-'));
}

function writeTranscript(homeDir, projectSlug, fileName, entries) {
  const transcriptDir = path.join(homeDir, '.gemini', 'projects', projectSlug);
...
function runTests() {
  ...
  if (test('reports malformed JSONL lines as an attention signal', () => {
    ...
    try {
      const transcriptDir = path.join(homeDir, '.gemini', 'projects', '-Users-affoon-project-malformed');
      fs.mkdirSync(transcriptDir, { recursive: true });
      fs.writeFileSync(
  ...
  if (test('watch mode emits repeated JSON status frames', () => {
    ...
    try {
      ...
      assert.strictEqual(frames[0].schemaVersion, 'egc.loop-status.v1');
      assert.strictEqual(frames[1].schemaVersion, 'egc.loop-status.v1');
  ...
  if (test('writes per-session status snapshots and index when write-dir is set', () => {
    ...
    try {
      ...
      assert.strictEqual(stdoutPayload.schemaVersion, 'egc.loop-status.v1');
      ...
      assert.strictEqual(indexPayload.schemaVersion, 'egc.loop-status.index.v1');
      ...
      assert.strictEqual(snapshotPayload.schemaVersion, 'egc.loop-status.session.v1');
