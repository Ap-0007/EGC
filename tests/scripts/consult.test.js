...
function runTests() {
  ...
  if (test('shows help with an explicit help flag', () => {
    ...
    assert.match(result.stdout, /Consult EGC install components/);
  ...
  if (test('recommends security components and profile for a natural language query', () => {
    ...
    assert.strictEqual(payload.target, 'egc');
    ...
    assert.strictEqual(
      ...
      'npx egc install --profile minimal --target egc --with capability:security'
    );
  ...
  if (test('prints text recommendations with install and plan commands', () => {
    ...
    assert.strictEqual(result.status, 0, result.stderr);
    assert.match(result.stdout, /EGC consult/);
    assert.match(result.stdout, /capability:security/);
    assert.match(result.stdout, /npx egc install --profile minimal --target egc --with capability:security/);
    assert.match(result.stdout, /npx egc plan --profile minimal --target egc --with capability:security/);
  })) passed++; else failed++;

  if (test('works from outside the EGC repository', () => {
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'egc-consult-project-'));
    try {
