...
const syncScript = path.join(repoRoot, 'scripts', 'sync-egc-to-codex.sh');
const deterministicPackageEnv = {
  GEMINI_PACKAGE_MANAGER: 'npm',
...
if (
  test('merge-mcp-config no-ops after all recommended servers are present', () => {
    ...
    try {
      ...
      assert.match(second.stdout, /All EGC MCP servers already present/);
  ...
  test('merge-mcp-config removes disabled legacy servers without appending replacements', () => {
    ...
    try {
      ...
      const result = runNode(mergeMcpConfigScript, [configPath], {
        ...
        EGC_DISABLED_MCPS: allServersDisabled,
      });

      assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
      assert.match(result.stdout, /Disabled via EGC_DISABLED_MCPS/);
