...
function runTests() {
  ...
  (test('creates metrics file when given valid usage data', () => {
    ...
    const metricsFile = path.join(tmpHome, '.gemini', 'metrics', 'costs.jsonl');
  ...
  (test('handles missing usage fields gracefully', () => {
    const tmpHome = makeTempDir();
    const input = { model: 'egc-sonnet-4-20250514' };
    const inputStr = JSON.stringify(input);
    const result = runScript(input, withTempHome(tmpHome));
    assert.strictEqual(result.code, 0, `Expected exit code 0, got ${result.code}`);
    assert.strictEqual(result.stdout, inputStr, 'Expected stdout to match original input');
  ...
  // 6. Canonical EGC_SESSION_ID wins; legacy ECC_SESSION_ID is fallback only (Phase 9)
  (test('prefers EGC_SESSION_ID over legacy ECC_SESSION_ID when both are present', () => {
    const tmpHome = makeTempDir();
    const input = {
      model: 'egc-sonnet-4-20250514',
    ...
    const result = runScript(input, {
      ...
      EGC_SESSION_ID: 'egc-session-9999',
    });
    assert.strictEqual(result.code, 0, `Expected exit code 0, got ${result.code}`);

    const metricsFile = path.join(tmpHome, '.gemini', 'metrics', 'costs.jsonl');
    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'egc-session-9999', 'Expected canonical EGC_SESSION_ID to win');

    fs.rmSync(tmpHome, { recursive: true, force: true });
  }) ? passed++ : failed++);

  // 6b. Legacy ECC_SESSION_ID still works as a fallback when EGC_SESSION_ID is unset
  (test('falls back to legacy ECC_SESSION_ID when EGC_SESSION_ID is unset', () => {
    const tmpHome = makeTempDir();
    const input = {
      model: 'egc-sonnet-4-20250514',
      usage: { input_tokens: 120, output_tokens: 30 },
    ...
    const metricsFile = path.join(tmpHome, '.gemini', 'metrics', 'costs.jsonl');
    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'egc-session-legacy', 'Expected legacy ECC_SESSION_ID fallback');
