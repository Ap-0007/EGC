...
function readMetricRows(homeDir) {
  const metricsFile = path.join(homeDir, '.gemini', 'metrics', 'tool-usage.jsonl');
...
function runTests() {
  ...
  (test('passes through input on stdout', () => {
    ...
    const result = runScript(input, {
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
  ...
  (test('creates tool activity metrics rows with file paths', () => {
    ...
    const result = runScript(input, {
      ...
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-1234',
    });
    assert.strictEqual(result.code, 0);

    const metricsFile = path.join(tmpHome, '.gemini', 'metrics', 'tool-usage.jsonl');
    assert.ok(fs.existsSync(metricsFile), `Expected metrics file at ${metricsFile}`);

    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'egc-session-1234');
  ...
  (test('captures typed move file events from source/destination inputs', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-5678',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('captures replacement diff previews for edit tool input', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-edit',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('captures MultiEdit nested edits with typed diff previews', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-multiedit',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('reclassifies tracked Write activity as modify using git diff context', () => {
    ...
    spawnSync('git', ['config', 'user.email', 'egc@example.com'], { cwd: repoDir, encoding: 'utf8' });
    spawnSync('git', ['config', 'user.name', 'EGC Tests'], { cwd: repoDir, encoding: 'utf8' });
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-write-modify',
    }, {
      cwd: repoDir,
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('captures tracked Delete activity using git diff context', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-delete',
    }, {
      cwd: repoDir,
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('resolves repo-relative paths even when the hook runs from a nested cwd', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-nested-cwd',
    }, {
      cwd: nestedCwd,
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('prefers canonical EGC_SESSION_ID over legacy ECC_SESSION_ID and redacts bash summaries', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-1',
      EGC_SESSION_ID: 'egc-session-2',
    });
    assert.strictEqual(result.code, 0);

    const metricsFile = path.join(tmpHome, '.gemini', 'metrics', 'tool-usage.jsonl');
    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'egc-session-2', 'canonical EGC_SESSION_ID must win when both are present');
  ...
  (test('falls back to legacy ECC_SESSION_ID when canonical EGC_SESSION_ID is unset', () => {
    ...
    const input = {
      tool_name: 'Bash',
      tool_input: { command: 'echo legacy-compat' },
      tool_output: { output: 'done' },
    };
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-legacy',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('skips non-PostToolUse events and rows without required identifiers', () => {
    assert.strictEqual(buildActivityRow(
      ...
      { GEMINI_HOOK_EVENT_NAME: 'PreToolUse', ECC_SESSION_ID: 'sess' }
    ), null);
    assert.strictEqual(buildActivityRow(
      { tool_name: 'Read', tool_input: { file_path: 'README.md' } },
      { GEMINI_HOOK_EVENT_NAME: 'PostToolUse' }
    ...
    assert.strictEqual(buildActivityRow(
      { tool_input: { file_path: 'README.md' } },
      { GEMINI_HOOK_EVENT_NAME: 'PostToolUse', ECC_SESSION_ID: 'sess' }
    ), null);
  }) ? passed++ : failed++);
  ...
  (test('sanitizes nested params, long summaries, and output variants', () => {
    ...
    const row = buildActivityRow(
      ...
      { GEMINI_HOOK_EVENT_NAME: 'PostToolUse', EGC_SESSION_ID: 'egc-fallback' }
    );

    assert.strictEqual(row.session_id, 'egc-fallback');
  ...
  (test('records creation previews unchanged when running outside a git repository', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-non-git-create',
    }, {
      cwd: tmpCwd,
    });
  ...
  (test('preserves absolute paths outside the repo without git enrichment', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      GEMINI_HOOK_EVENT_NAME: 'PostToolUse',
      ECC_SESSION_ID: 'egc-session-absolute-outside',
    });
  ...
  (test('passes empty stdin through without creating metrics', () => {
    ...
    assert.strictEqual(
      fs.existsSync(path.join(tmpHome, '.gemini', 'metrics', 'tool-usage.jsonl')),
      false
    );
