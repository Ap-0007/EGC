...
def generate_scenarios(
    ...
    Calls egc -p with the scenario_generator prompt, parses YAML output.
    """
    ...
    result = subprocess.run(
        ["egc", "-p", prompt, "--model", model, "--output-format", "text"],
        capture_output=True,
        text=True,
        timeout=120,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(f"egc -p failed: {result.stderr}")

    if not result.stdout.strip():
        raise RuntimeError("egc -p returned empty output")
