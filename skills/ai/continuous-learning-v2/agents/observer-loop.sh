...
ANALYSIS_COOLDOWN="${EGC_OBSERVER_ANALYSIS_COOLDOWN:-${ECC_OBSERVER_ANALYSIS_COOLDOWN:-60}}"
IDLE_TIMEOUT_SECONDS="${EGC_OBSERVER_IDLE_TIMEOUT_SECONDS:-${ECC_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${EGC_OBSERVER_ALLOW_WINDOWS:-${ECC_OBSERVER_ALLOW_WINDOWS:-false}}" != "true" ]; then
    echo "[$(date)] Skipping egc analysis on Windows due to known non-interactive hang issue (#295). Set EGC_OBSERVER_ALLOW_WINDOWS=true (or legacy ECC_OBSERVER_ALLOW_WINDOWS=true) to override." >> "$LOG_FILE"
    return
  fi

  if ! command -v egc >/dev/null 2>&1; then
    echo "[$(date)] egc CLI not found, skipping analysis" >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${EGC_OBSERVER_MAX_ANALYSIS_LINES:-${ECC_OBSERVER_MAX_ANALYSIS_LINES:-500}}"
...
PROMPT
  ...
  # resolution, so relying on cat "$prompt_file" inside the egc invocation
  ...
  timeout_seconds="${EGC_OBSERVER_TIMEOUT_SECONDS:-${ECC_OBSERVER_TIMEOUT_SECONDS:-120}}"
  max_turns="${EGC_OBSERVER_MAX_TURNS:-${ECC_OBSERVER_MAX_TURNS:-20}}"
  exit_code=0
  ...
  # EGC_* is the canonical skip guard; ECC_* is set in parallel as the legacy
  # bridge (never removed) so older observe.sh variants / downstream consumers
  # still recognise the guard. The model is symbolic and resolver-driven via
  # EGC_OBSERVER_MODEL (legacy: ECC_OBSERVER_MODEL) - never a pinned versioned
  # model ID; "flash" maps to the current fast Gemini model in the registry.
  ...
  EGC_SKIP_OBSERVE=1 EGC_HOOK_PROFILE=minimal ECC_SKIP_OBSERVE=1 ECC_HOOK_PROFILE=minimal \
    egc --model "${EGC_OBSERVER_MODEL:-${ECC_OBSERVER_MODEL:-flash}}" --max-turns "$max_turns" --print \
  ...
  (
    ...
    if kill -0 "$egc_pid" 2>/dev/null; then
      echo "[$(date)] Gemini analysis timed out after ${timeout_seconds}s; terminating process" >> "$LOG_FILE"
      kill "$egc_pid" 2>/dev/null || true
    fi
  ) &
  watchdog_pid=$!

  wait_for_egc_analysis "$egc_pid"
  exit_code=$?
