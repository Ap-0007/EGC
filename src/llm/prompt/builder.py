...
# Provider adapters are optional at import time: their SDKs (anthropic, openai,
# google-genai) may not be installed. The prompt builder itself does not need
# them, so import resiliently and keep the names available for any re-export.
try:
...
class PromptBuilder:
    def __init__(self, config: PromptConfig | None = None, **config_overrides) -> None:
        # Accepts either a ready PromptConfig or individual config keyword
        # overrides (e.g. PromptBuilder(include_tools_in_system=True)). Explicit
        # keyword overrides win over fields of a supplied config.
        if config is None:
            self.config = PromptConfig(**config_overrides) if config_overrides else PromptConfig()
        elif config_overrides:
            from dataclasses import replace as _replace
            self.config = _replace(config, **config_overrides)
        else:
            self.config = config
